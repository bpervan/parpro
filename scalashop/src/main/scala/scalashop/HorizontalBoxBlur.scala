...
object HorizontalBoxBlur extends HorizontalBoxBlurInterface {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    ...
    val taskSize = (src.height.toFloat / numTasks.toFloat).ceil.toInt
    val tasks = (0 until src.height by taskSize).zip((0 to src.height by taskSize).tail).map(pair => task{

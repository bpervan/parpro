...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val ret = new Boundaries()
    ret.maxX = math.max(a.maxX, b.maxX)
    ret.maxY = math.max(a.maxY, b.maxY)
    ret.minX = math.min(a.minX, b.minX)
    ret.minY = math.min(a.minY, b.minY)
    ret
  }
  ...
  def computeSectorMatrix(bodies: coll.Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))((secMat, body) => secMat += body, (a, b) => a.combine(b))
  }
  ...
  def updateBodies(bodies: coll.Seq[Body], quad: Quad): coll.Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    //parBodies.map(body => body.updated(quad))
    parBodies.foldLeft(Seq[Body]())((accu, elem) => accu :+ elem.updated(quad))
  }
